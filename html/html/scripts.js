...
async function generateIdCard(data) {
  ...
  for (const [i, employee] of data.entries()) {
    ...
    const logoUri = await getDataUri(employee.logo,  400, 400);
    doc.addImage(logoUri, 0.807, 0.15, 0.51, 0.51);
    ...
    doc.text(employee.gymname, 1.0625, 0.790, null, null, 'center');
    ...
    employeeInfo.forEach((item, i) => {
      doc.text(item.label, 0.3, 2.450 + 0.167 * i);
      doc.text(':', 0.76, 2.450 + 0.167 * i);
      doc.text(item.value, 0.813, 2.450 + 0.167 * i);
    });
    ...
    doc.rect(0, 1.68, 2.125, 1.697, 'F');
    doc.setFillColor('#fff');
     doc.rect(0.658, 1.000, 0.833, 0.833, 'F');
    var qrCodeUri = '/image/GYM00001.jpg';
    //getQrCodeUri(employee.id); 
    doc.addImage(qrCodeUri, 0.670, 1.000, 0.820, 0.820);
